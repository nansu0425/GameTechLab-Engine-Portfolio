...
이후 Mesh BVH 의 분할 기준도 Scene BVH 와 동일하게 Binned SAH (32 bin, 3축 평가) 로 보강했습니다. 위 0.031 ms 측정은 보강 전 (longest-axis + centroid median split) 시점의 결과이며, SAH 적용 후 별도 측정은 진행하지 않았습니다. SAH 선택 실패 시 longest-axis median split 으로 fallback 합니다. mesh asset 당 1회만 빌드되어 모든 instance 가 공유하므로, 빌드 비용 자체는 picking 런타임 hot path 와 분리됩니다. ([커밋 e98d24d](https://github.com/nansu0425/GameTechLab-WEEK05/commit/e98d24d))
...
| 결정 | 채택 | 대안 | 채택 이유 / 비용 |
|------|------|------|----------------|
| 분할 기준 (Scene / Mesh BVH 공통) | Binned SAH (32 bin) | Midpoint split | SAH 는 트래버설 비용 기댓값을 직접 최소화 → 트래버설 시간 단축. Scene BVH 는 Dirty Refit, Mesh BVH 는 asset 당 1회 빌드 + instance 공유로 빌드 비용을 상쇄. |
